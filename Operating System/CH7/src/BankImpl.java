...
public class BankImpl implements Bank {
    ...
    public BankImpl(int[] resources) {
        ...
        // initialize the resources array to have the same arrayLength as source array
        available = new int[m];
     ...
     * private int n: the number of threads in the system
     *
     * @param threadNum 0 ~ (m-1)
     * @param request request of resources of Customer # threadNum
    ...
    private boolean isSafeState(int threadNum, int[] request) {
        ...
        for (int i = 0; i < m; i++)
            if (request[i] > available[i]) {
                System.err.print("INSUFFICIENT RESOURCES for Customer # "+threadNum);
                return false;
            }

        // ok, there are sufficient reources. Now let's see if we can find an ordering of threads to finish
        ...
        /**
         * Now try to find an ordering of threads so that each thread can finish.
         * private int m: the number of resources
         * private int n: the number of threads in the system
         */
        for (int i = 0; i < n; i++) {
            // find the first thread that can finish
            for (int j = 0; j < n; j++){
                if (!canFinish[j]) {
                    boolean temp = true;
                    // need of m resources
                    ...
                    if (temp) { // if this thread can finish
                        canFinish[j] = true;
                        for (int x = 0; x < m; x++)
                            // restore the temporarily deduced avail of resource x
        ...
        for (int i = 0; i < n; i++)
            if (!canFinish[i]) {
                returnValue = false;
                System.err.print("UNSAFE STATE - CAN'T FIND AN ORDERING OF THREADS TO FINISH ALL");
                break;
            }
        return returnValue;
    }
    ...
    public synchronized boolean requestResources(int threadNum, int[] request) {
        if (!isSafeState(threadNum, request)) {
            System.out.print(" Customer # " + threadNum + " is denied.");
            return false;
        }

        // isSafeState = true, all threads can complete in an order
        ...
        System.out.print("ResourceAllocated = ");
        for (int i = 0; i < m; i++)
            System.out.print(allocation[threadNum][i] + "  ");


        System.out.print(" Remaining Need = ");
        for (int i = 0; i < m; i++)
